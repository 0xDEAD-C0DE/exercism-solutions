pub fn answer(command: &str) -> Option<i32> {
    ...
    if command.len() == 1 {
        return Some(command.first().unwrap().parse::<i32>().unwrap());
    }
    if command.is_empty() {
        return None;
    }
    ...
    let mut result: i32;
    // Check if the first item of the command is a number. If not,
    // dont do anything.
    if command.first().unwrap().parse::<i32>().is_ok() {
        result = command.first().unwrap().parse::<i32>().unwrap();
    } else {
        return None;
    }
